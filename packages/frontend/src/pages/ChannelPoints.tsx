...
export function ChannelPointsPage() {
  ...
  const uploadFile = async (type: "sound" | "image" | "video", file: File): Promise<{ url: string; duration?: number }> => {
      ...
      {editorAlertData && (
        <OverlayEditor
          alert={{
            ...
            ttsEnabled: false, ttsVoice: "", ttsRate: 1.0, ttsVolume: 80, videoMuted: editorAlertData.action.videoMuted ?? false, videoLoop: editorAlertData.action.videoLoop ?? false,
          }}
...
function ActionCard({ action, index, commands, channelId, onUpdate, onRemove, onUploadFile, onOpenLayout }: {
  ...
  onUploadFile: (type: "sound" | "image" | "video", file: File) => Promise<{ url: string; duration?: number }>; onOpenLayout: () => void;
}) {
  return (
...
function SoundActionForm({ action, onUpdate, onUploadFile }: { action: RewardActionSound; onUpdate: (p: Partial<RewardActionSound>) => void; onUploadFile: (t: "sound"|"image"|"video", f: File) => Promise<{ url: string; duration?: number }> }) {
...
function AlertActionForm({ action, onUpdate, onUploadFile, onOpenLayout }: { action: RewardActionAlert; onUpdate: (p: Partial<RewardActionAlert>) => void; onUploadFile: (t: "sound"|"image"|"video", f: File) => Promise<{ url: string; duration?: number }>; onOpenLayout: () => void }) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Sounds {(() => { const s = action.soundFileUrl?.split(",").filter(Boolean) ?? []; return s.length > 1 ? <span className="text-muted-foreground font-normal">(zufällig)</span> : null; })()}</Label>
          ...
          <div className="mt-1"><FileUpload accept=".mp3,.wav,.ogg,.webm" label={action.soundFileUrl ? "+ Sound" : "Upload Sound"} onUpload={async (f) => { const { url: u } = await onUploadFile("sound", f); if (u) { const existing = action.soundFileUrl ? action.soundFileUrl + "," + u : u; onUpdate({ soundFileUrl: existing }); } }} /></div>
        </div>
      </div>
      {action.imageFileUrl && /\.(webm|mp4)$/i.test(action.imageFileUrl) && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex items-center justify-between"><Label>Video wiederholen</Label><Switch checked={action.videoLoop ?? false} onCheckedChange={(v) => onUpdate({ videoLoop: v })} /></div>
        </div>
      )}
